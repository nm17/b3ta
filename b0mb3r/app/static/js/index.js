...
document.querySelector("#main-form").addEventListener("submit", async (e) => {
    ...
    let rawResponse = await fetch("/attack/start", {
        method: 'POST',
        headers: {'Content-Type': 'application/json'},
        body: JSON.stringify({number_of_cycles: document.querySelector('#count').value, phone: phone}),
    });
    let content = await rawResponse.json();

    const interval = setInterval(async () => {
        await fetch("/attack/" + content.id + "/status", {
            method: 'GET'
        }).then((response) => {
